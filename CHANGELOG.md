...
- **Severe excursions now override cooldown (all advisors).** Cooldown exists to avoid re-dosing before the last change takes effect — but it shouldn't keep the advisor idle while a parameter is *badly* out of band. New `cooldown_override_pct` (default 50%): when the median sits more than that far beyond the nearest target-band edge, cooldown is bypassed and a recommendation is made (still step-capped and floor-guarded, so it can't over-correct). The reason text flags `⚠ Cooldown overridden`. Set to 0 to restore always-hold-in-cooldown. Example: PO4 at 0.15 ppm against a 0.06 ceiling is 150% over → advises a reduction immediately instead of waiting out the cooldown.

Five new tests in `tests/test_advisor.py` (remover empirical potency: matches-spec, diverges-triggers-warning, wrong-sign-rejected; severe-excursion cooldown override + override-disabled) plus updated NO3/PO4 defaults assertions.
